# AuNP_Mie_TMM_Simulation
This work presents a semi-analytical simulation framework combining Mie scattering theory and Transfer Matrix Method (TMM) to optimise an etched Corning single-mode (SMF) 28 optical fiber decorated with gold nanoparticles (AuNPs) for localized surface plasmon resonance (LSPR) based detection of cadmium, lead and mercury ions in aqueous medium. 
...
`transfer\\\_matrix.py`	Slab waveguide TMM for etched SMF-28	I_ev(delta), l_d(delta)
`sensitivity\\\_map.py`	Composite 2D parametric sensitivity	S_total(delta, d)
...
`fig1\\\_extinction\\\_spectra`	Mie extinction Q_ext for d = 20-80 nm AuNPs	Single col.
`fig2\\\_sensitivity\\\_diameter`	LSPR sensitivity and FOM vs diameter	Single col.
`fig3\\\_evanescent\\\_field`	I_ev and l_d vs etch depth (log scale)	Single col.
`fig3b\\\_mode\\\_profile`	Schematic fiber cross-section at delta=30 µm	Single col.
`fig4\\\_sensitivity\\\_map`	MAIN FIGURE — 2D composite S_total(delta, d)	Double col.
...
`fig7\\\_sensitivity\\\_lod\\\_tradeoff`	LOD vs S_total for design guidance	Single col.
...
Instrument noise: sigma_lambda = 0.01 nm (Ocean Insight USB4000)
LOD = 3*sigma_lambda / S_total (3-sigma criterion)
Citation
